import React from "react";
import HeroCarousel from "./HeroCarousel";
import HeroFeature from "./HeroFeature";
import Image from "next/image";
// import { serverFetch } from "@/utils/helper/serverFetch";
import HeroRightBanner from "./HeroRightBanner";

const Hero = async ({
  slider,
  rightBanner
}) => {

  // const banners: any[] = await serverFetch(`/api/v1/banners/active`, { requireAuth: false });

  // console.log('banners==>', banners)

  // const heroRightBanners = banners.filter(i => i.position == 'HOME_TOP_RIGHT_TOP' || i.position == 'HOME_TOP_RIGHT_BOTTOM').slice(0, 2);
  // const heroLeftBanners = banners.filter(i => i.position == 'HOME_TOP_LEFT_SLIDER');

  return (
    <section className="overflow-hidden pb-10 lg:pb-12.5 xl:pb-15 pt-57.5 sm:pt-45 lg:pt-30 xl:pt-51.5 bg-[#E5EAF4]">
      <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
        <div className="flex flex-wrap gap-5">
          <div className="xl:max-w-[757px] w-full">
            <div className="relative z-1 rounded-[10px] bg-white overflow-hidden">
              {/* <!-- bg shapes --> */}
              <Image
                src="/images/hero/hero-bg.png"
                alt="hero bg shapes"
                className="absolute right-0 bottom-0 -z-1"
                width={534}
                height={520}
              />

              <HeroCarousel slides={slider} />
            </div>
          </div>

          <div className="xl:max-w-[393px] w-full">
            <div className="flex h-full flex-col sm:flex-row xl:flex-col justify-between gap-5">
              {rightBanner && rightBanner?.map(item => (
                <HeroRightBanner
                  key={item?._id}
                  title={item.title}
                  subtitle={item.subtitle}
                  image={item.image}
                  price={item?.sliderMeta?.product?.price}
                  discountedPrice={item?.sliderMeta?.product?.discountedPrice}
                  redirectUrl={item.redirectUrl}
                />
              ))
              }
            </div>
          </div>
        </div>
      </div>

      {/* <!-- Hero features --> */}
      <HeroFeature />
    </section>
  );
};

export default Hero;
